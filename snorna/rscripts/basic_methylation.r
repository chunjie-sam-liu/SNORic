...
database_meth_expression <- paste("snorna_methylation", str_to_lower(str_replace(dataset_id, "TCGA-", "")), sep = "_")
...
# mrna_expression
res_meth_expression <- dbSendQuery(con, statement = paste(
                       "SELECT * FROM ", database_meth_expression,
                       "WHERE snorna = ", paste('"',q_name,'"', sep=''),
                       "AND dataset_id = ",
                       paste('"',dataset_id,'"', sep = ""), sep=" "))
expr_meth_expression <- dbFetch(res_meth_expression, n = -1) %>% as_tibble()
dbClearResult(res_meth_expression)
...
if(nrow(expr_snorna_expression) < 20 || mean(expr_snorna_expression$snorna_expression) < 1){
    write_json(NULL, path = file.path(resource_jsons, paste(arg_encode, 'methylation', 'json', sep=".")))
    print("Not enough samples OR mean expression less than 1!")
    quit("no", status = 0)
}

corr_data <-
    expr_meth_expression%>%
...
corr_data %>%
        ...
        corr_data_json
write_json(corr_data_json, path = file.path(resource_jsons, paste(arg_encode, 'methylation', 'json', sep=".")))

corr_data_distinct <- unique(corr_data$meth_id)
plot_point <- function(x){
    ...
    ggsave(filename = file.path(resource_pngs, paste(arg_encode,x,"methylation","png",sep = ".")),plot=p, device = "png")
}
