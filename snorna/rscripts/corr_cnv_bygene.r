...
table_cnv <- str_c("snorna_cnv", str_to_lower(str_replace(dataset_id, "TCGA-", "")), sep = "_")
...
if(nrow(pair_info) < 1){
    write_json(
        ...
        path = file.path(
                    resource_jsons,
                    str_c(arg_encode, 'corr_cnv_bygene', 'json', sep="."))
                    )
    quit("no", status = 0)
}
...
cnv <- tbl(db, table_cnv) %>%
    collect(n = Inf)
# wrangling data
corr_data <-
    snorna_expr%>%
    inner_join(cnv, by = c("snorna", "dataset_id", "sample_id")) %>%
...
corr_data %>%
    ...
    write_json(path = file.path(resource_jsons, str_c(arg_encode, 'corr_cnv_bygene', 'json', sep=".")))
