...
var snoric=(function(){
    ...
    var default_datatable_settings = {
            ...
            columns: [
                ...
                {data: null,
                    render: function(data, type, row){
                        return row["p.value"] > 0.01 ? row["p.value"]: row["p.value"].toExponential(2).replace(/e\+0|e\-0/, "");
                    }},
                {
        ...
        corr_geneexpr_datatable_settings = {
            columns: [
                ...
                {data: null,
                render: function(data, type, row){
                    return row["p_value"] > 0.01 ? row["p_value"]:row["p_value"].toExponential(2).replace(/e\+0|e\-0/, "");
                }},
                {
                    class: 'details-control',
                    orderable: false,
        ...
        corr_splicing_data_settings = {
            columns: [
                ...
                render: function(data, type, row){
                    // return row["p"];
                    return row["p"] > 0.01 ? row["p"]:row["p"].toExponential(2).replace(/e\+0|e\-0/, "");
                }},
                {
                    class: 'details-control',
                    orderable: false,
        ...
        rnaexpr_datatable_settings = {
            ...
            columns: [
                ...
                {data: null,
                    "width": "10%",
                    render: function(data, type, row){
                        var d = Number(Math.log(row.snorna_expression + 1) / Math.log(2));
                        return d.toExponential(2).replace(/e\+0|e\-0/, "");
                    }
                }
            ]
        },
        ...
        order: [[2, 'desc']],
            columns: [
                ...
                render: function(data, type, row){
                    return Number(Math.log(row.mean_expression + 1) / Math.log(2)).toExponential(2).replace(/e\+0|e\-0/, "");
                }}
            ]
        };
    ...
    function toggleDataTableRow() {
        function format(table_id, data) {
            var img = "/SNORic/basic/" + table_id + "/png/" + data.encode.replace("/","#");
            switch(table_id){
